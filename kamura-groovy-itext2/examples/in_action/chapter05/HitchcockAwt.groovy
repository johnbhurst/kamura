...
new IText2Builder(new FileOutputStream("build/examples/in_action/chapter05/HitchcockAwt1.pdf")).document(rect) {
  image(imageLocation("hitchcock.png"), init: {it.setAbsolutePosition(15, 15)})
}

new IText2Builder(new FileOutputStream("build/examples/in_action/chapter05/HitchcockAwt2.pdf")).document(rect) {
  image(awtImage: awtImage, color: null, init: {it.setAbsolutePosition(15, 15)})
}

new IText2Builder(new FileOutputStream("build/examples/in_action/chapter05/HitchcockAwt100.pdf")).document(rect) {
  image(writer: writer, awtImage: awtImage, quality: 1, init: {it.setAbsolutePosition(15, 15)})
}

new IText2Builder(new FileOutputStream("build/examples/in_action/chapter05/HitchcockAwt20.pdf")).document(rect) {
  image(writer: writer, awtImage: awtImage, quality: 0.2, init: {it.setAbsolutePosition(15, 15)})
}

new IText2Builder(new FileOutputStream("build/examples/in_action/chapter05/HitchcockAwt10.pdf")).document(rect) {
